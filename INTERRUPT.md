...
| second answer | 90c | A run's result reaches the parent when that run concludes | The parent's half is done and identical either way, proven on both branches 2026-08-16. The agent's half is the whole remainder of that row and it does not exist for a `--bg` agent — "so the rest of 90c is actually a INTERRUPT thing". i4 |
...
| | i4 | A run's outcome takes the mic from the agent — the rest of 90c | "agent gets to wake in the 15th scond of the first sleep" / "coinflip GREEN, dont care, still abruptly notifies the agent" / "agent has no choice here" / "and it doesnt matter if it passed or not, we care about the result, always, unconditionally" | The row this document is named for. `interrupt`, then the outcome as a `user` line. No branch on green versus red anywhere in it. The acceptance test is coinflip: FLIP1 dispatches, SLEEP1 is 30s, the run takes about 15, and the agent is interrupted at 15 rather than told at 30 |
