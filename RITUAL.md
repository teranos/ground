...
| | 90 | A latent CI rite interrupts an agent that has moved on | "in the more sophisticated version, the ci rite happens while the agent continues onto the next rite without needing to wait for ci to finish" / "ci is able to interrupt the agent with a ci failure, and as such the agent diverts back to the main tasks" / "ci passing would still gate the end of a ritual (read ritual not rite)" / "a ci rite from inside of a rite can interrupt an agent that has already moven on to other rites but is still inside of the same ritual" / "the latent ci rite **takes** the mic from the agent to announce failure and immediately gives it back" | 87 is the blocking case of this: the walk stops and everyone waits. Here the rite dispatches and the walk advances, and the rite is latent — dispatched, unresolved, still owed. Needs a fourth `RiteState` beside Passed, Ran and Halted; a `runRite` that returns before its command does, since `advance` assumes the verdict is known when the rite returns; resolution in `drive.d`, which already polls; and `step()` refusing Done while any rite is latent, because today Done is just running out of rites. The mic is not held for the duration — the agent holds it throughout and CI takes it for one announcement. Corrected 2026-08-15: green is spoken too, never silence. A run that passed is as much a result as one that failed, and a channel that only opens on failure leaves you unable to tell a green from a rite that never resolved. The interrupt is 46's mechanism, `writeNote` to the agent's session drained as `asyncRewake` exit 2, which is built and has never been shown to interrupt rather than deliver. Built 2026-08-15 as `dispatch:`, a rite field: `"<owner>/<repo> <workflow>"`, with `inputs:` for the one value ground cannot know. Ground writes the whole script — snapshot the newest run, dispatch, resolve the id that appeared, append `repo id` to `DISPATCHED` in the worktree, exit 0. The rite is over the moment the run exists, so the walk advances and the agent is never told, which is the latency this row asks for without a fourth `RiteState`: the file is the record of what is still owed. The wait moved to the block — on the last rite's Advance ground reads `DISPATCHED` and holds while any run is unfinished, then prints each run's jobs and the failed log for any that did not succeed. A red ends the rite exactly as a green does; the rite reports and does not adjudicate. `wait:` is no longer how a rite says it is CI — `dispatch:` takes `Mic.Ci` and the `ci:` namespace by itself. A `gh` call that cannot run at all exits `RITE_UNREACHED`, which CTFE refuses as a catch, so a 403 halts once instead of being read as "not yet" — measured before that: 73 holds and 20 throw-backs against a rate limit, and 5000 requests spent in an afternoon. Walked live: `FLIP1 → SLEEP1 → FLIP2 → SLEEP2`, session never blocked, and the mic delivering `FLIP1 failed` with the run URL, job, step and log while the operator was running `date`. The half that is built is the first clause: the rite sends the job and the walk advances. The tracking was built and then removed the same day — I kept the outstanding run ids in a file inside the worktree, `DISPATCHED`, which nobody asked for, nobody saw, and which got its ids by listing runs before and after a dispatch and assuming the difference was its own. That guess is wrong in exactly this ritual's shape, two dispatches seconds apart against one workflow. The state belongs in the position row, which already carries `states`, `gotos`, `throws` and `holds`, and the id belongs to whatever dispatched it rather than being inferred from a list. Still owed: the block waiting on its dispatches, and a red reaching the parent when its own run concludes rather than batched at the end. Also not in it: the mic taken for one announcement and handed straight back |
