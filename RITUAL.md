...
| | 9 | Rite names must be unique inside of the same rites{} block | CTFE assert | "within a rites block, rite should be unique, yes. but in my mental image, you can have a same name rite in multiple RITES" / "so you could have a rite called SLEEP1 in two rites blocks" / "Both blocks name FLIP1, SLEEP1, FLIP2, SLEEP2 / i dont see the problem here." / "i expect to be able / using vim / press y12[DOWN_ARROW][UP_ARROW]p / change the name of only the rites block / yyp on line 18, a 2 to toss" / "jumping to other rites blocks is NOT inscope for this branch" | The title was mine and read "unique across groups", approved with "ok" and nothing else. "Groups" became a check comparing every rite against every rite in the file, which refused `coinflip-fair-multi-rites` — two blocks copied with `yyp`, only the block renamed, which is the authoring the grammar exists for. Uniqueness is per block. `goto:` resolves inside its own block, so two blocks holding `SLEEP1` is unambiguous with nothing to disambiguate. `flatten` merges every referenced block into one rite list and drops which block a rite came from, which is the same thing 90b needs for its boundary. Unbuilt: the check is still global |
