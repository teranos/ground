...
| | 90d | CI takes the mic for one announcement and hands it straight back | "the latent ci rite **takes** the mic from the agent to announce failure and immediately gives it back" / "ci is able to interrupt the agent with a ci failure, and as such the agent diverts back to the main tasks" / "a ci rite from inside of a rite can interrupt an agent that has already moven on to other rites but is still inside of the same ritual" | The mic is not held for the duration — the agent holds it throughout. The interrupt is 46's mechanism, `writeNote` to the agent's session drained as `asyncRewake` exit 2, which is built and has never been shown to interrupt rather than deliver |
