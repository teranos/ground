...
| ✓ | 9 | Rite names must be unique inside of the same rites{} block | CTFE assert | "within a rites block, rite should be unique, yes. but in my mental image, you can have a same name rite in multiple RITES" / "so you could have a rite called SLEEP1 in two rites blocks" / "Both blocks name FLIP1, SLEEP1, FLIP2, SLEEP2 / i dont see the problem here." / "i expect to be able / using vim / press y12[DOWN_ARROW][UP_ARROW]p / change the name of only the rites block / yyp on line 18, a 2 to toss" / "jumping to other rites blocks is NOT inscope for this branch" | The title was mine and read "unique across groups", approved with "ok" and nothing else. "Groups" became a check comparing every rite against every rite in the file, which refused `coinflip-fair-multi-rites` — two blocks copied with `yyp`, only the block renamed, which is the authoring the grammar exists for. Uniqueness is per block. `goto:` resolves inside its own block, so two blocks holding `SLEEP1` is unambiguous with nothing to disambiguate. `flatten` keeps `group` on every rite, so the block a rite came from was never dropped — that is what 90b's boundary reads. Built 2026-08-17: the comparison is per block, and `coinflip-fair-multi-rites` walked `FLIP1,SLEEP1,FLIP2,SLEEP2` twice over from `toss` and `toss2`, having refused to compile an hour before. Not built, and not this branch: `indexOfRite` still returns the first match by name, so a `goto:` naming a rite that two blocks both hold jumps to the first one — no pbt does that today |
...
| ✓ | 90b | A rites block does not finish while a dispatch it made is outstanding, and the next block does not start | "a rites block will not complete until each of its rite dispatch has been completed" / "in my mental model rites toss is over when both dispatches are over" / "dispatch getting a result gates the RITES block" / "RITE_B does not start until RITE_A has completely finished" / "a RITUAL is not finished until each of its rites blocks finished" / "a dispatch gates a rites block from finishing until it results something, a result" | The gate is at every block boundary, not only at the end of the ritual: a ritual is a list of rites-block references, and each is finished before the next begins. `flatten` merges every referenced block into one rite list, so the walk cannot see where one ends — carrying those boundaries through is the work, and the dispatch-result check is the easy half. coinflip is one block, so it can demonstrate the end-of-ritual case and not block A gating block B: `FLIP2` sends a run of about fifteen seconds, `SLEEP2` sleeps three, and the walk runs out of rites with a result still owed. Recorded here 2026-08-17 from session text; the row previously led with "ci passing would still gate the end of a ritual (read ritual not rite)", which answers to no transcript and which I repeated back as yours. Built 2026-08-17: `lastOfBlock` reads the boundary off the `group` that `flatten` already keeps on every rite, and `outstandingDispatch` counts dispatch rows with no receipt, scoped to the performance by the token prefix. The gate sits before the rite rather than after its verdict — the block is the unit that waits, and after the verdict a `run:` re-fired on every cycle and the agent was woken with nothing to answer. Walked on `coinflip-fair-multi-rites-1786994635`: `toss` sat at SLEEP2 with T1FLIP2 in flight, released the moment its result landed, `toss2` did not start early, and `done` came only after T2FLIP1 answered — `holds 0`, no agent turn spent waiting. The run before it, with the gate in the wrong place, cost `holds 6` and four agent messages saying nothing. Built and removed the same day. I kept the outstanding run ids in `DISPATCHED`, a file inside the worktree that nobody asked for and nobody saw, and got the ids by listing runs before and after a dispatch and assuming the difference was its own. That guess is wrong in exactly this ritual's shape — two dispatches seconds apart against one workflow. The state belongs in the position row, which already carries `states`, `gotos`, `throws` and `holds`; the id belongs to whatever dispatched it rather than being inferred from a list. Today Done is just running out of rites |
| | 90c | A run's result reaches the parent when that run concludes | "failure needs to prop to parent immediately when i happens" / "if FLIP1 ci is red, parent should know about it on T15 and the FLIP2 ci red would surface on T2 + T15" | Per run, not batched at the end of the block. Corrected 2026-08-15: green is spoken too, never silence — a run that passed is as much a result as one that failed, and a channel that only opens on failure leaves you unable to tell a green from a rite that never resolved. Half done as of 2026-08-16, and the other half cannot be done here — "90c is only halfish done, because we cant do better on a --bg agent". The parent's half runs: `writeDispatchStatus` records that an outcome is owed the instant the job is accepted, `checkRunByToken` finds the run by the name ground gave it, and the result comes back per run as that run concludes. Watched on both branches the same evening — `CI: success long-coin coinflip-heads-1786912085:FLIP1` and `:FLIP2`, `CI: failure long-coin coinflip-tails-1786912112:FLIP1` and `:FLIP2`, the reds carrying `--log-failed`. The agent's half is not a channel that is missing, it is an interrupt, and a `--bg` agent has no stdin to take the mic through. INTERRUPT.md i4 |
...
| ✓ | 97 | A rite may not write its own exit code | "ground uses those exit codes, becaue not all failures are the same failure" / "\|\| TRUE \|\| EXIT 1 \|\| EXIT 22 \|\| EXIT WHATEVER SHOUDL NOT BE A THING IN GROUND" / "i dont want \|\| true ANYWHERE" / "NOT IN GROVE NOT IN SUN NOT IN MOON NOT ANYWHERE" | Everything ground does with a rite reads the code, and the grammar accepted rites that erased it. `launders` refuses `\|\| true`, `\|\| :` and `\|\| exit` anywhere in an `eval:` or `run:`, at CTFE, naming the rite — the same shape as the `${` refusal that predates it. Thirteen sites across five pbt files were rewritten to let the tool answer: four `gh pr create … \|\| true` that made a rate limit, an auth failure and already-open into one event, and nine `test … \|\| exit 1` where the code was redundant under the `set -euo pipefail` every rite already runs with. Proven by planting one in grove's START and watching the build refuse it. `validateRituals` stopped concatenating in the same pass — `~` allocates and this project has no GC, so eleven messages compiled only while nothing emitted the function as runtime code |
| ✓ | 98 | A command reaching for a file gets the file | "you can use curl instead of gh+jq" is elsewhere; this is "substitute_for_read" | A deny takes the method away and leaves the goal unmet, and an agent handed that stops reading rather than reading properly — watched in another session, where a refused `sed` became a long exchange about a file nobody had opened. `substitute_for_read: ["sed", "awk", "perl"]` names the utilities that mean *I was trying to read*; ground opens the file and hands it over, and the command does not run. `readTargets` is pinned by nineteen commands taken from `ground shovel PreToolUse` over two weeks rather than invented — pipes hand over nothing, `elapsed` and `unused` and `sed_placeholder` are not sed, perl takes several files at once, and `crowbar "sed …"` reads a file on another machine. Proven live: `sed -n '1,12p' SUBAGENT.md` returned the whole file |
