...
| D | 87 | CI holds the mic while a run is in flight | "there needs to be a way for the ball to land in the court of ci" / "and ci keeps holding the mic in this case" / "lets say after APPLE you coinflip by menas of the 15s ci job" / "and if ci passes it lets you jump" | Deferred. Two things are missing and one already exists. `goto` fires only on a caught code — `run.d:50` is `wantsJump = a.verdict == Verdict.Hold` — so a jump on a pass cannot be written, and expressing it would mean inverting the rite to pass on the failure. Nothing hands the mic to `Mic.Ci`, so a rite waiting on a run reads as the agent holding, which is the lie the mic exists to remove. What exists: `checkCIStatus` with its four outcomes, and `adaptive.d` keeping p50/p90 per repo and branch, which is already the bound `micBound` wants for `Mic.Ci`. grove has `short-moon` at 3s always green and `long-coin` at 15s failing half the time, so both a slow green and a real red can be walked |
