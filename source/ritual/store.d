...
// How long a rite may read as running before another walker takes it anyway.
// Longer than a bounded dispatch, short enough that a walker killed mid-rite
// does not park the walk.
enum WALK_LEASE_SEC = 120;

// The other half of the guard: a walker holding a row another has already
// advanced has nothing left to run, whatever the lease says.
bool claimsOneExecution(long rowRev, long expectedRev) {
    return rowRev == expectedRev;
}

// The rule the SQL below mirrors, as something CTFE can assert on.
bool canClaim(const(char)[] heldBy, long heldAt, const(char)[] me,
              long now, long leaseSec) {
    if (heldBy.length == 0) return true;
    if (heldBy == me) return true;
    return heldAt <= now - leaseSec;
}

// Claim the right to run the current rite. The guarded UPDATE is the claim:
// sqlite serialises it, so two walkers racing cannot both see a row changed.
// A lease alone only serialises: the second walker waits, then runs the same
// rite anyway. `rev` is what makes it a claim on one execution — a walker
// holding a row another has already advanced has nothing left to run.
bool claimWalk(DB)(DB db, const(char)[] id, const(char)[] me, long now,
                   long expectedRev) {
    import db : sqlite3_prepare_v2, sqlite3_step, sqlite3_finalize,
                sqlite3_bind_text, sqlite3_bind_int64, sqlite3_changes,
                sqlite3_stmt, SQLITE_OK, SQLITE_TRANSIENT;

    if (id.length == 0 || me.length == 0) return false;
    enum sql = "UPDATE ritual_position SET walker = ?2, walker_at = ?3 "
        ~ "WHERE id = ?1 AND state = 'live' AND rev = ?5 "
        ~ "AND (walker = '' OR walker = ?2 OR walker_at <= ?4)\0";

    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, sql.ptr, -1, &stmt, null) != SQLITE_OK) return false;
    sqlite3_bind_text(stmt, 1, id.ptr, cast(int) id.length, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 2, me.ptr, cast(int) me.length, SQLITE_TRANSIENT);
    sqlite3_bind_int64(stmt, 3, now);
    sqlite3_bind_int64(stmt, 4, now - WALK_LEASE_SEC);
    sqlite3_bind_int64(stmt, 5, expectedRev);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);
    return sqlite3_changes(db) == 1;
}

// Let go of it. Guarded on the holder so a walker cannot release another's.
void releaseWalk(DB)(DB db, const(char)[] id, const(char)[] me) {
    import db : sqlite3_prepare_v2, sqlite3_step, sqlite3_finalize,
                sqlite3_bind_text, sqlite3_stmt, SQLITE_OK, SQLITE_TRANSIENT;

    if (id.length == 0 || me.length == 0) return;
    enum sql = "UPDATE ritual_position SET walker = '' "
        ~ "WHERE id = ?1 AND walker = ?2\0";
